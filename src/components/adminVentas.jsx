            ...
            <table className="table-fixed">
                <thead className="text-left bg-orange-400 text-gray-700">
                    <tr>
                        ...
                        <th className="px-2">Fecha del Pedido</th>
                        <th className="px-2">Fecha de Entrega</th>
                        <th className="px-2">Subtotal</th>
                        <th className="px-2">IGV</th>
                ...
                <tbody>
                    {data.map((ventas, index) => (
                        <tr
                            key={index}
                            className={`${
                                ventas.status ? "bg-orange-200" : "bg-gray-400"
                            }`}
                        >
                            <td className="px-5">{ventas.id}</td>
                            <td className="px-2">{ventas.usuario}</td>
                            <td className="px-2">
                                {
                                    new Date(ventas.fechaPedido)
                                        .toISOString()
                                        .split("T")[0]
                                }
                            </td>
                            <td className="px-2">
                                {
                                    new Date(ventas.fechaEntrega)
                                        .toISOString()
                                        .split("T")[0]
                                }
                            </td>
                            <td className="px-2">S/. {ventas.subtotal}</td>
                            ...
                            <td className="px-2">
                                {ventas.status ? (
                                    <button
                                        ...
                                        title="Dar de Baja"
                                        onClick={() => deleteVentas(ventas.id)}
                                    >
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            viewBox="0 0 24 24"
